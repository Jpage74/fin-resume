...
输入 JD + 简历 → 智能工具调用 → 四环处理管线 → 输出结构化匹配报告：

1. **智能工具调用（防幻觉第一道门）** → LLM 先判定消息类型再调工具：简历→`set_resume`、JD→`analyze_jd`、一般提问→不调工具直接回答；工具 docstring + 指令含"违反即为幻觉"的硬规则，拿不准时主动向用户确认
2. **JD 解析** → 硬门槛校验（学历 / 专业 / 证书 / 实习要求）
3. **RAG 案例库检索** → 匹配岗位 + 上岸者背景画像（相似度阈值把关，标注出处 `source=`）
4. **证据化差距分析** → 每项判断绑定简历证据；证据不足标「待证据」，不编造
5. **reviewer 复核** → 二次校验，拦截幻觉

> 反幻觉闭环：任何匹配度、硬门槛、source 引用、needs_proof 标记只能来自 `analyze_jd` 返回的报告原文，
> 禁止模型绕过工具自行编造结论。
